    ...
    try {
...

        {
            ...
            if ($search_result->isOk()) {
                ...
                $index = (int)getVarFromUser('Enter the index');
                foreach ($search_result->getItems() as $r) {
                    $tracks = $search_result->getItems();
                    $tracks = $tracks[$index];
                    $track = $tracks->getTrack();
                    $title_name = $track->getTitle();
                    $audio_id = $track->getAudioClusterId();
                    }
                }
            }
        output("the track with index of\t". $index . "\tis\n :". $track);
        ...
        $first_retry = true;
         {
            ...
            $is_connected_fail = false;
            {
                ...
                try {
                    $feed = $ig->reels->getMusicFeed($audio_id, $maxId);
                    $is_connected = true;
                } catch (\InstagramAPI\Exception\NetworkException $e) {
