...
class Story extends RequestCollection
{
     ...
     * @param string $container_module    (optional) From which app module (page) you're performing this action.
     * 
     * @throws \InstagramAPI\Exception\InstagramException
     *
    ...
    public function like(
        ...
        $nav_chain = '1ne:feed_timeline:1:cold_start,ReelViewerFragment:reel_feed_timeline:5:button')
    {
        if ($this->ig->getPlatform() === 'ios') {
            if ($container_module == 'feed_contextual_profile') {
                $nav_chain = 'IGMainFeedViewController:feed_timeline:1:cold_start,IGProfileViewController:profile:2:media_owner,IGStoryViewerViewController:reel_profile:3:';
            } else {
                $nav_chain = 'IGMainFeedViewController:feed_timeline:1:cold_start,IGStoryViewerViewController:reel_feed_timeline:7:';
            }
        }
        ...
        return $this->ig->request('story_interactions/send_story_like/')
            ->addPost('media_id', $mediaId)
            ->addPost('source_of_like', 'button')
            ->addPost('tray_session_id', Signatures::generateUUID(true))
            ->addPost('_uid', $this->ig->account_id)
            ->addPost('_uuid', $this->ig->uuid)
            ->addPost('viewer_session_id', Signatures::generateUUID(true))
        ...
        $nav_chain = '1ne:feed_timeline:1:cold_start,ReelViewerFragment:reel_feed_timeline:5:button')
    {
        ...
        return $this->ig->request('story_interactions/unsend_story_like/')
            ->addPost('media_id', $mediaId)
            ->addPost('source_of_like', 'button')
            ->addPost('tray_session_id', Signatures::generateUUID(true))
            ->addPost('_uid', $this->ig->account_id)
        ...
        $reason = 'pull_to_refresh')
    {
        if ($this->ig->getPlatform() === 'android') {
            $this->ig->setNavChainHeader('1ne:feed_timeline:1:cold_start', true);
        } else {
            $this->ig->setNavChainHeader('IGMainFeedViewController:feed_timeline:1:cold_start', true);
        }

        if ($this->ig->getPlatform() === 'android') {
            $supported_capabilities_new = Constants::SUPPORTED_CAPABILITIES;
        } else {
            $supported_capabilities_new = Constants::IOS_SUPPORTED_CAPABILITIES;
        }
        
        return $this->ig->request('feed/reels_tray/')
            ->setSignedPost(false)
            ->addPost('supported_capabilities_new', json_encode($supported_capabilities_new))
            ...
            ->addPost('_uuid', $this->ig->uuid)
            ->addPost('_uid', $this->ig->account_id)
            ->addPost('page_size', 25) 
            ->getResponse(new Response\ReelsTrayFeedResponse());
    }

    /**
     * Get the global story feed which contains everyone you follow (web API)
     *
     * Note that users will eventually drop out of this list even though they
     * still have stories. So it's always safer to call getUserStoryFeed() if
     * a specific user's story feed matters to you.
     ...
     * @see Story::getUserStoryFeed()
     */
    public function getReelsTrayFeedGraph()
    {
        $request = $this->ig->request('feed/reels_tray/')
            ...
            ->addHeader('X-Asbd-Id', $this->ig->settings->get('asbd_id'))
        ...
        $userId)
    {
        if ($this->ig->getPlatform() === 'android') {
            ...
            $this->ig->setNavChainHeader('IGMainFeedViewController:feed_timeline:1:cold_start,IGStoryViewerViewController:reel_feed_timeline:7:', true);
        }
    ...
    /**
     * Get multiple users' story feeds as stream (or specific highlight-details) at once.
     * 
     * NOTE: Normally, you would only use this endpoint for stories (by passing
     * UserPK IDs as the parameter). But if you're looking at people's highlight
     * feeds (via `Highlight::getUserFeed()`), you may also sometimes discover
    ...
    /** 
     * Turn OFF auto share story to Facebook
     *
     * @throws \InstagramAPI\Exception\InstagramException
     *
     * @return \InstagramAPI\Response\GenericResponse
     */
    public function turnOffAutoShareToFacebook()
    {
        return $this->ig->request("/ig_fb_xposting/user_setting/update/")
            ->addPost('_uid', $this->ig->account_id)
            ->addPost('_uuid', $this->ig->uuid)
            ->addPost('xpost_setting_status_to_update', 'false')
            ->getResponse(new Response\GenericResponse());
    }
    
    /** 
     * Turn ON auto share story to Facebook
     *
     * @throws \InstagramAPI\Exception\InstagramException
     *
     * @return \InstagramAPI\Response\GenericResponse
     */
    public function turnOnAutoShareToFacebook()
    {
        return $this->ig->request("/ig_fb_xposting/user_setting/update/")
            ->addPost('_uid', $this->ig->account_id)
            ->addPost('_uuid', $this->ig->uuid)
            ->addPost('xpost_setting_status_to_update', 'true')
            ->getResponse(new Response\GenericResponse());
    }
