...
class Collection extends RequestCollection
{
    ...
    /**
     * Get saved collections for media
     *
     * @param string      $mediaPk The media PK.
     * 
     * @throws \InstagramAPI\Exception\InstagramException
     *
    ...
    public function getMediaCollections(
        $mediaPk)
    {
        return $this->ig->request("save/media/{$mediaPk}/saved_collections_for_media/")
        ...
        $moduleName = 'feed_saved_add_to_collection')
    {
        return $this->ig->request('collections/create/')
            ...
            ->addPost('cover_media_id', $cover_media_id)
        ...
        array $params)
    {
        ...
        if (!empty($params['remove_media']) && is_array($params['remove_media'])) {
            $postData['removed_media_ids'] = json_encode(array_values($params['remove_media']));
        }
