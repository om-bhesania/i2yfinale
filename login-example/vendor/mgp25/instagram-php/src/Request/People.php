...
class People extends RequestCollection
{
        ...
        $module = null)
    {
        if ($this->ig->getPlatform() === 'ios') {
            $nav_chain = 'IGMainFeedViewController:feed_timeline:1:cold_start';
        } else {
            $nav_chain = '1ne:feed_timeline:1:cold_start';
        }
    ...
    {
        if (empty($this->ig->client->_navChain)) {
            $this->ig->setNavChainHeader('1wP:explore_popular:2', true);
        }
        ...
        $enable_groups = true)
    {
        if ($this->ig->getPlatform() === 'ios') {
            if ($userId == $this->ig->account_id) {
                $nav_chain = 'IGProfileViewController:self_profile:14:main_profile';
            } else {
                $nav_chain = 'IGMainFeedViewController:feed_timeline:1:cold_start,IGProfileViewController:profile:25:media_owner,IGFollowListTabPageViewController:unified_follow_lists:26:';
            }
        } else {
            $nav_chain = '1wP:explore_popular:2,UserDetailFragment:profile:3,ProfileMediaTabFragment:profile:4,C6N:following:5';
        }
        ...
        $enable_groups = true)
    {
        if ($this->ig->getPlatform() === 'ios') {
            if ($userId == $this->ig->account_id) {
                $nav_chain = 'IGProfileViewController:self_profile:14:main_profile,IGFollowListTabPageViewController:self_unified_follow_lists:22:';
            } else {
                $nav_chain = 'IGMainFeedViewController:feed_timeline:1:cold_start,IGProfileViewController:profile:25:media_owner';
            }
        } else {
            $nav_chain = '1wP:explore_popular:2,UserDetailFragment:profile:3,ProfileMediaTabFragment:profile:4,C6N:following:5';
        }
    ...
    public function follow(
        ...
        $nav_chain = '1wP:explore_popular:2,UserDetailFragment:profile:3,ProfileMediaTabFragment:profile:4,ProfileFollowRelationshipFragment:following_sheet:5,ProfileMediaTabFragment:profile:6')
    {
        if ($this->ig->getPlatform() === 'ios') {
    ...
    public function unfollow(
        ...
        $nav_chain = '42U:self_profile:10,ProfileMediaTabFragment:self_profile:11,5dS:self_following:12,5dS:self_following:13')
    {
        if ($this->ig->getPlatform() === 'ios') {
            if ($container_module == 'self_following') {
                $nav_chain = 'IGProfileViewController:self_profile:14:main_profile,IGFollowListTabPageViewController:self_unified_follow_lists:18:';
