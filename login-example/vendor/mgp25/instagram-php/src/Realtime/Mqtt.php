...
use InstagramAPI\Realtime\Subscription\GraphQl\AsyncAdSubscription;
use InstagramAPI\Realtime\Subscription\GraphQl\ClientConfigUpdateSubscription;
use InstagramAPI\Realtime\Subscription\GraphQl\DirectStatusSubscription;
use InstagramAPI\Realtime\Subscription\GraphQl\IgLiveWaveSubscription;
use InstagramAPI\Realtime\Subscription\GraphQl\InteractivityActivateQuestionSubscription;
use InstagramAPI\Realtime\Subscription\GraphQl\InteractivityRealtimeQuestionSubmissionsStatusSubscription;
use InstagramAPI\Realtime\Subscription\GraphQl\InteractivitySubscription;
use InstagramAPI\Realtime\Subscription\GraphQl\LiveRealtimeCommentsSubscription;
use InstagramAPI\Realtime\Subscription\GraphQl\LiveTypingIndicator;
...
class Mqtt implements PersistentInterface
{
        ...
        ExperimentsInterface $experiments)
    {
        ...
        $this->_irisEnabled = true;
        $this->_msgTypeBlacklist = true;
        $this->_mqttLiveEnabled = true;
        $this->_graphQlTypingEnabled = true;
        $this->_inboxPresenceEnabled = true;
        $this->_threadPresenceEnabled = true;
    }
    ...
    protected function _initSubscriptions()
    {
        ...
        if (($this->_inboxPresenceEnabled || $this->_threadPresenceEnabled) && empty($this->_additionalOptions['disable_presence'])) {
            $this->_logger->debug('Set up AppPresenceSubscription().');
            $this->_doAddSubscription($appPresenceSubscription, false);
        } else {
            $this->_doRemoveSubscription($appPresenceSubscription, false);
        }
        $subscriptionId = Signatures::generateUUID();
        $zeroProvisionSubscription = new ZeroProvisionSubscription($subscriptionId, $this->_auth->getDeviceId());
        $this->_logger->debug('Set up ZeroProvisionSubscription().');
        $this->_doAddSubscription($zeroProvisionSubscription, false);
        $subscriptionId = Signatures::generateUUID();
        $DirectStatusSubscription = new DirectStatusSubscription($subscriptionId);
        $this->_logger->debug('Set up DirectStatusSubscription().');
        $this->_doAddSubscription($DirectStatusSubscription, false);
        $subscriptionId = Signatures::generateUUID();
        $graphQlTypingSubscription = new DirectTypingSubscription($subscriptionId, $this->_auth->getUserId());
        if ($this->_graphQlTypingEnabled) {
            $this->_logger->debug('Set up DirectTypingSubscription().');
            $this->_doAddSubscription($graphQlTypingSubscription, false);
        } else {
            $this->_doRemoveSubscription($graphQlTypingSubscription, false);
        }
        $subscriptionId = Signatures::generateUUID();
        $AsyncAdSubscription = new AsyncAdSubscription($subscriptionId, $this->_auth->getDeviceId());
        $this->_logger->debug('Set up AsyncAdSubscription().');
        $this->_doAddSubscription($AsyncAdSubscription, false);

        // Skywalker Subscriptions
        $this->_logger->debug('Set up DirectSubscription().');
        $this->_doAddSubscription(new DirectSubscription($this->_auth->getUserId()), false);
        $this->_logger->debug('Set up LiveSubscription().');
     ...
     */
    protected function _receiveOfflineMessages()
    {
        $this->_logger->debug('Receive offline messages starting from the sequence ID.');
        try {
            $sequenceId = (int) $this->_auth->getSequenceId();
            $snapshotAtMs = number_format(microtime(true), 3, '', '');
            $snapshotAppVersion = \InstagramAPI\Constants::IG_IOS_VERSION;
            $command = new IrisSubscribe($sequenceId, $snapshotAtMs, $snapshotAppVersion);
