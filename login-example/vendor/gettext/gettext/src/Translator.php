...
class Translator extends BaseTranslator implements TranslatorInterface
{
    ...
    protected function getPluralIndex($domain, $n)
    {
        ...
        if (!isset($this->plurals[$domain]['function'])) {
            // $this->plurals[$domain]['function'] = create_function('$n', self::fixTerseIfs($this->plurals[$domain]['code'])); // deprecated function create_function()
            $this->plurals[$domain]['function'] = function($n) use ($domain) {
                return self::fixTerseIfs($this->plurals[$domain]['code']);
            };
        }
